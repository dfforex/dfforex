...
export async function getAuthorizedDerivAccount(token, appId = '') {
...
export async function buyRiseFallContract({ token, signal, stake = 1, duration = 5, durationUnit = 'm', currency = 'USD', appId = '' }) {
...
export async function syncDerivContract({ token, contractId, appId = '' }) {
  if (!token) throw new Error('Token Deriv ausente.');
