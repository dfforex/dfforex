...
export async function withDeriv(fn, options = {}) {
  const cfg = getConfig();
  const appId = String(options.appId || cfg.deriv.appId || cfg.deriv.legacyAppId || '1089').trim() || '1089';
  ...
  await new Promise((resolve, reject) => {
    ...
    ws.once('error', (err) => reject(new Error(`Falha ao abrir WebSocket Deriv com app_id=${appId}: ${err?.message || err}`)));
  });
  try {
...
export async function getCandles(symbol, granularitySeconds = 3600, count = 240, appId = '') {
...
export async function testDerivConnection(tokenOverride = '', appId = '') {
  return await withDeriv(async (client) => {
    ...
    return {
      ...
      app_id: client.cfg?.deriv?.appId || appId || '1089',
