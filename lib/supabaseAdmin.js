...
export async function insertLog(level, message, payload = {}) {
  const supabase = getSupabaseAdmin();
  if (!supabase) return { skipped: true, reason: 'Supabase não configurado' };
  const { data, error } = await supabase.from('bot_runtime_logs').insert({ level, message, payload }).select('id').single();
